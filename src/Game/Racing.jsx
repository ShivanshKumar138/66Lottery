...
const GameRulesModal = ({ open, handleClose }) => {
  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ px: 2, py: 2 }}> {/* Reduced padding */}
        <Typography variant="body2" paragraph sx={{ mb: 1, fontSize: '0.875rem' }}>
          1 Minute 1 issue, 55 Seconds To Order, 5 Seconds Waiting for the draw. It Opens all day. The Total number of Trade is 1440 issues.
        </Typography>
        <Typography variant="body2" paragraph sx={{ mb: 1, fontSize: '0.875rem' }}>
          If you spend 100 to trade, after deducting 2.5 Service fee, Your Contract Amount is 97.5:
        </Typography>
        
        <Box component="ol" sx={{ pl: 2, mt: 0 }}>
          <Typography component="li" sx={{ mb: 0.5, fontSize: '0.875rem' }}>
            <strong>Select Green:</strong> If the result shows 1,3,7,9 You will get (97.5×2)195; If the Result shows 5 You Will Get (97.5×1.5) 146.25
          </Typography>
          
          <Typography component="li" sx={{ mb: 0.5, fontSize: '0.875rem' }}>
            <strong>Select Red:</strong> If The result shows 2,4,6,8 You Will Get (97.5×2)195; If the result shows 0, You Will Get (97.5×1.5) 146.25
          </Typography>
          
          <Typography component="li" sx={{ mb: 0.5, fontSize: '0.875rem' }}>
            <strong>Select Violet:</strong> If the Result shows 0 Or 5, You Will Get (97.5×4.5)438.75
          </Typography>
          
          <Typography component="li" sx={{ mb: 0.5, fontSize: '0.875rem' }}>
            <strong>Select Number:</strong> If The Result is the Same As the Number You Selected, You Will Get (97.5×9)877.5
          </Typography>
          
          <Typography component="li" sx={{ mb: 0.5, fontSize: '0.875rem' }}>
            <strong>Select Small:</strong> If The Result shows 0,1,2,3,4 You Will Get (97.5×2)195
          </Typography>
          
          <Typography component="li" sx={{ mb: 0.5, fontSize: '0.875rem' }}>
            <strong>Select Big:</strong> If The Result shows 5,6,7,8,9, You Will Get (97.5×2)195
          </Typography>
        </Box>
      </DialogContent>
...
useEffect(() => {
  ...
  const handlePlaceBet = async () => {
    ...
    if (
      ["00:06", "00:05", "00:04", "00:03", "00:02", "00:01"].includes(remainingTime)
    ) {
  ...
  <Typography variant="caption" sx={{ color: "white", fontSize: "14px", mt: 1 }}>
    ...
    <div className="mt-4 bg-gray-900 relative">
        ...
        <div className="relative h-60 overflow-hidden">
          ...
          {/* Track lanes */}
          <div className="absolute bottom-0 left-0 right-0 z-3" style={{ height: '35%' }}>
            {[...Array(10)].map((_, i) => (
              <div 
                key={i} 
          ...
          {/* Traffic light */}
          <div className="absolute left-1/2 top-1/3 transform -translate-x-1/2 bg-black p-2 rounded-lg flex gap-1 z-20">
            <div className={`w-8 h-8 rounded-full ${isRacing || (remainingTime <= "00:03" && remainingTime > "00:00") ? 'bg-green-500' : 'bg-gray-700'}`}></div>
            <div className={`w-8 h-8 rounded-full ${!isRacing && remainingTime <= "00:06" && remainingTime > "00:03" ? 'bg-yellow-500' : 'bg-gray-700'}`}></div>
            <div className={`w-8 h-8 rounded-full ${!isRacing && remainingTime > "00:06" ? 'bg-red-600' : 'bg-gray-700'}`}></div>
          </div>
          
          {/* Finish line */}
          <div className="absolute left-6 bottom-4 h-32 w-10 flex flex-col z-10">
            {[...Array(10)].map((_, i) => (
              <div key={i} className={i % 2 === 0 ? 'h-3 bg-black' : 'h-3 bg-white'}></div>
            ))}
          </div>
      ...
      {/* Below betting option */}
          ...
          >
            {/* Timer Overlay */}
                     ...
                     <div className="mt-1 p-2">
                     <div className="flex justify-between mb-4 w-full">
                        <button className={`py-2 px-11 rounded-full flex-grow mx-1 ${activeTabNumber === 0 ? "bg-orange-500 text-white" : "bg-white text-orange-500 border border-orange-500"
                        }`}
                        onClick={() => handleTabClick(0)}
                        >
                            1st
                        ...
                        <button className={`py-2 px-11 rounded-full flex-grow mx-1 ${activeTabNumber === 1 ? "bg-orange-500 text-white" 
                                : "bg-white text-orange-500 border border-orange-500"
                            }`}
                            onClick={() => handleTabClick(1)}
                        >
                        ...
                        <button className={`py-2 px-11 rounded-full flex-grow mx-1 ${activeTabNumber === 2? "bg-orange-500 text-white" : "bg-white text-orange-500 border border-orange-500"
                            }`}
                            onClick={() => handleTabClick(2)}
                        >
                            3rd
